 ...
 * over stdio, hand-rolled (plan decision 1).
 *
 * WHY hand-rolled, not `@modelcontextprotocol/sdk`: this repo's posture is one
 * runtime dep (`yaml`). The SDK drags zod and a transport stack for a surface we
 * need only a few hundred lines of. The hub side already proves interop against
 * the official SDK client, and the `--mcp` e2e tests here drive a real
 * client-side handshake, so drift is tested rather than assumed.
 *
 ...
 * long `whats_next` park is outstanding, `notifications/cancelled` must reach
 * the server to abort it (D12 cancel-mid-park) and `ping` must still answer.
 * Serializing the pump behind the in-flight call chain would deadlock exactly
 * those frames on the real transport. Per-frame ordering still holds for the
 * non-parking methods because `handleLine` runs synchronously up to the first
 * tool-handler await, and responses are matched by id, not by order.
 *
...
export function pumpStdin(stream: LineStream, server: McpServer, onEof?: () => void): void {
  ...
  const finish = (): void => {
    ...
    // all resolve on cancel (whats_next) or are bounded hub calls (hold tools).
