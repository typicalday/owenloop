...
test('exitCodeFor maps every outcome to the documented code', () => {
  ...
  const one: ExecOutcome[] = ['misroute', 'workdir-denied', 'workdir-missing', 'unresolved-instructions', 'killed', 'lease-lost', 'ownership-error', 'hub-unreachable', 'submit-rejected', 'submit-failed', 'command-failed', 'ask-failed', 'judge-no-verdict', 'reject-failed', 'stopped'];
