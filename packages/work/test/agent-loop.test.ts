...
test('final response evidence is redacted from regular worker logs', async () => {
  const secret = 'SENSITIVE_FINAL_RESPONSE_MUST_NOT_REACH_THE_WORKER_LOG';
  const adapter = createFakeAdapter({
    start: { events: [{ kind: 'assistant_response', text: secret }, { kind: 'turn_ended' }] },
  });
  ...
  assert.ok(h.errs.includes('owenloop work agent-run: final response received (redacted)'));
