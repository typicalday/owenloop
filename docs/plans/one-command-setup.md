...
**Chosen approach — "Option B".** The plugin marketplace ships *inside* the `owenloop` npm package. `owenloop setup` points each harness's plugin CLI at a directory inside its own install tree. No second repo, no git fetch, and the plugin version is structurally incapable of drifting from the CLI version.
...
1. **A `SessionStart` hook** — Claude Code supports it; Codex supports it (see §2.6) subject to the hook-trust gate. Runs `owenloop --version`, compares to the plugin's own version, prints an actionable message on mismatch. Catches "CLI missing" and "CLI too old" before any tool call and before the transport can fail opaquely. Note the plugin currently ships **only** `SessionEnd`; the former `SessionStart` hook was deliberately removed, so this re-adds one with a new purpose.
...
12. Add a `SessionStart` hook to **both plugin trees** (revision 4) that runs `owenloop --version`, compares against the plugin's own version, and prints an actionable message on mismatch or on the binary being absent. On Codex the hook runs only after the user approves it via `/hooks` (item 14); on Claude Code it runs after install with no extra step. It must never block session start. Its non-redundant value over the in-tool check: when `owenloop` is missing from PATH or too old to have the `mcp` subcommand, the MCP server never starts, no tool call ever happens, and this hook is the only surface that can print a useful remedy. Note the Claude Code pack deliberately deleted its old `SessionStart` hook; this is a new hook with a different purpose, not a revival — say so in the commit message.
