...
A restricted Codex run uses a positive `codexConfig` allowlist. The exact workflow-authored allowlist is:

- with `filesystem: workspace-write`, `codexConfig.sandbox_workspace_write.exclude_slash_tmp` may be a boolean;
- with `filesystem: workspace-write`, `codexConfig.sandbox_workspace_write.exclude_tmpdir_env_var` may be a boolean.

No other `codexConfig` top-level or nested key is accepted under `read-only` or `workspace-write`; unknown and future keys are refused. The refusal includes `model_provider`, `model_providers` (and provider `base_url` or `env_key` definitions inside that map), `openai_base_url`, `chatgpt_base_url`, `mcp_servers`, notification commands, hooks, plugins, `project_root_markers`, `sandbox_workspace_write.network_access`, and `sandbox_workspace_write.writable_roots`. The separate `mcpServers` extension cannot declare external servers in restricted mode. `filesystem: unrestricted` preserves the existing open-ended `codexConfig` and `mcpServers` behavior.
