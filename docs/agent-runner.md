...
A restricted Codex run uses an empty workflow-authored `codexConfig` allowlist. Under `read-only` or `workspace-write`, `codexConfig` may be absent or `{}`, but any top-level key is refused. The refusal includes `model_provider`, `model_providers` (and provider `base_url` or `env_key` definitions inside that map), `openai_base_url`, `chatgpt_base_url`, `mcp_servers`, notification commands, hooks, plugins, `project_root_markers`, and the complete `sandbox_workspace_write` map. A restricted workflow therefore cannot author either temporary-directory exclusion, `network_access`, or `writable_roots`. The separate `mcpServers` extension cannot declare external servers in restricted mode. `filesystem: unrestricted` preserves the existing open-ended `codexConfig` and `mcpServers` behavior.

The worker-created `owenloop` MCP mount is adapter-owned rather than workflow-authored. The adapter adds or overwrites the mount only after restricted workflow config has passed the allowlist. The same sanitizer removes non-allowlisted config inherited by `thread/resume`, including provider endpoints and credential-source fields, before the adapter adds the current Owenloop mount.

Per-thread `config.mcp_servers` merges with the operator's Codex configuration instead of replacing it. For `read-only` and `workspace-write`, the adapter starts the app-server with a stable, config-free `CODEX_HOME` under the Owenloop cache. The isolated home links only the operator's `auth.json`; global config, MCP servers, plugins, hooks, providers, and endpoints are not inherited. The cwd-derived isolated path persists Codex rollout files so a later worker process can resume the same thread. `danger-full-access` keeps the operator's normal Codex home because that mode claims no filesystem restriction.

Codex 0.146.0 has no reliable app-server switch that disables project-local config, and the isolated `CODEX_HOME` does not stop `thread/start` from loading a project `.codex/config.toml`. Before either cold start or resume, a restricted Codex run therefore checks the normalized and canonical cwd ancestor chains through the detected `.git` project root. After configuration sanitization, the adapter sets `config.project_root_markers` to `['.git']`; workflow-authored `codexConfig.project_root_markers` is refused and inherited values are removed. If any `.codex/config.toml` exists on either checked chain, the adapter refuses before spawning the app-server. Remove or rename the project-local `.codex/config.toml`, or use `filesystem: unrestricted` only when the workflow may trust that config. Unrestricted runs preserve custom project-root markers and continue to load project config normally.

Under `workspace-write`, the adapter reconstructs `sandbox_workspace_write` after all workflow and inherited configuration. The final map always contains `exclude_slash_tmp: true` and `exclude_tmpdir_env_var: true`, so neither `/tmp` nor the directory named by `$TMPDIR` is a writable sandbox root. A restricted workflow cannot disable either exclusion. The adapter sets `network_access: true` only for neutral `network: unrestricted` and never carries an authored or inherited `writable_roots` value. A restricted resume removes a broader turn's inherited workspace-write expansion before adding the reconstructed map.

These two exclusions are based on a provider-free local command-sandbox probe against the pinned Codex 0.146.0 binary. For each field, the probe found that omission and literal `false` leave the corresponding temporary-directory root writable; only literal `true` excludes that root. The committed tests therefore assert the fully serialized cold-start and resume parameters, including inherited `false` values being replaced with adapter-owned `true` values.
