...
When enabled, the timer measures silence between raw SDK messages, including partial stream events, rather than total turn time. Partial payloads are not logged or persisted. Recovery applies only to an order with exactly one owed output: it permits a primary turn, one same-session wake, and one cold replay. A submit or other hub outcome wins every race. Authentication, permission-policy, model-unavailable, and configuration failures go straight to a producer-side `ask`; exhausted recovery also holds through `ask` rather than releasing the debt.
