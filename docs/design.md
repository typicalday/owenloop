...
`maintainCalls` runs OUTSIDE any open `store.tx()`. Each mutating action (spawn via `spawnChildIfAbsent`, re-provide via `provideInput`, machine-green) opens its own `store.tx()`. No nested transactions — node:sqlite does not support nested `BEGIN IMMEDIATE`. The spawn's check-then-insert runs inside a *single* `BEGIN IMMEDIATE` (§23.6.9), so the reverse-lookup read and the child insert are one atomic step rather than two.
