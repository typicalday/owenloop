...
- The child's own producer re-arms on the normal knock-back loop — the verdict text rides the child producer's `owes` thread on its next firing, exactly like any other rejected artifact. Driving that re-arm requires ticking the CHILD instance (see the sweeping note below); `rejectCallsArtifact` also prompts `maintainCalls(parentWf)` so the parent's own `owed`/pin state is visible immediately, but it does not — and cannot, from the parent side alone — cause the child's producer step to fire.
...
**Sweeping note.** A propagated reject re-arms work in the CHILD instance, not the parent — `tick(parentWf)` re-runs `maintainCalls` (the mirror), but the child's own re-firing needs `tick(childWf)`. A driver/conductor that only ticks parents (never sweeping children) will see the parent sit `owed` indefinitely with no order ever appearing, because the actual work order is a child-instance concern. Drivers are expected to sweep every live instance (parent and child alike) — `store.listChildrenByParent`/`findChildByParent` give the reverse index needed to discover children from a parent id. The propagated reject is visible for such a sweep via the CHILD's own `status(childWf).debts` (the rejected outcome artifact appears there with `acceptance: 'rejected'`), independent of whether the sweep follows `producedBy` links or just iterates every known workflow id.
