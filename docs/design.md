...
- **Debt/done correctness**: an owed calls: artifact is a normal debt. It stays owed until the child's declared output is green with a value **and the whole child workflow is done** under the same rule as `workflowStatus().done` (including the satisfied-`atLeastOne` exception). Only then does the mirror green, so the parent cannot report done while the child still owes cleanup.
- **Model-checker abstraction**: the checker has no child-instance graph, so its one synthetic calls transition is the opaque completion of the whole child plus exposure of the declared output. It already models the terminal handoff rather than an early output event; the runtime join timing changes here require no new checker transition or cross-definition recursion.

### §23.4 Cross-def calls-cycle check

At `loadDefs` time, after all defs are expanded and per-def validated, `finalizeDefs` runs the original bounded first-cycle DFS and throws `DefError: calls cycle: a -> b -> a` on its first witness. Authoring commands call `reportCallsCycles(defs)`, which preserves that first witness and adds deterministic per-member witnesses until every cyclic definition is attributed, without enumerating the potentially exponential set of all simple cycles. Each finding names exactly the members on its displayed path. Both paths follow the same scope-aware resolution as runtime spawning and key each edge by the resolved definition, not by the bare text alone; two bundles may therefore each contain a workflow named `build` without being collapsed into one node.
...
4. **Outcome/join read**: read the child's declared `outputs:` artifact (exactly one, validated at load time) and compute whole-child done-ness from the same pinned child definition and artifact snapshot. Only a green, value-defined outcome from a done child may machine-green the parent's calls: artifact.
...
6. **Machine-green**: set parent calls: artifact to `acceptance: 'green'`, `version + 1`, `value = child outcome value`, `fingerprint = computeFingerprint(parentArts, gateStems) plus the child-outcome version pin` (§23.6.8, F4) — but only when the child is done and the child outcome's version has moved past whatever version is currently pinned on the parent artifact. Then `settle(parentWf)` so downstream (teardown) fires. Do NOT set `terminal` — the calls: artifact must be re-armable if gate inputs move or whole-child completion no longer holds.
7. **Re-arm on child working**: when the parent calls: artifact is green, re-arm it to `owed` if either the child's outcome is no longer green (e.g. re-provide re-armed it) **or the outcome remains green but the child is not done**. This handles gate re-arm and late cleanup correctly even though `deliver` declares `consumes: []` (the pure cascade cannot detect either cross-instance condition). The green-outcome/unfinished-child case records `called workflow is not done` and removes only the reserved child-output version pin, preserving every gate-version entry. Removing that one pin is what permits the unchanged declared output to be mirrored again after cleanup completes; ordinary non-green outcome re-arms retain the fingerprint, pin, and existing `gate input moved: child re-running` reason.

This widened re-arm is intentionally retroactive. On the next maintain pass, an existing store whose caller mirror is falsely green while its child is unfinished is reconciled back to `owed`; normal settling may invalidate downstream parent artifacts, and owed-only deep descent resumes the child's cleanup. This repairs persisted false completion without a schema or data migration. A child waiting for a manually supplied input remains an ordinary outstanding workflow; there is no timeout, bypass, warning, or load-time mapping rule.

#### §23.6.2 Cascade-up prompt

After a child `green` or `close`, `triggerParentIfChild(childWf)` reads the child's `producedBy` link and calls `maintainCalls(parentWf)` immediately instead of waiting for the next scheduled tick. This is a prompt to re-evaluate the join, not an unconditional outcome propagation: the parent mirror is published only when the declared child output is green and value-defined **and** `workflowDone(childDef, childArts)` is true for the whole child snapshot. If cleanup or a manually supplied input remains outstanding, the parent calls: artifact stays `owed`. Durability is free regardless: even without the prompt, the next parent tick calls `maintainCalls` and performs the same join check against persisted child state. The recursion guard (`_inMaintainCalls: Set<string>`) prevents `maintainCalls → provideInput → fireSettled → triggerParentIfChild → maintainCalls` infinite steps.

#### §23.6.3 `outputs:` as embedding interface

A workflow that can be called via `calls:` must declare exactly one `outputs:` stem (validated at `loadDefs` Phase 2). The called workflow's `outputs:[0]` selects the value to expose; greening that artifact is not by itself the completion signal. Its value is reflected to the parent's calls: artifact only when it is green and value-defined **and** `workflowDone(childDef, childArts)` says the whole child is done. The `delivery` workflow declares `outputs: [merge]` — its merge artifact is the public outcome. A parent `calls: delivery` eventually receives the merge value in its `delivered` artifact after all delivery work has completed.

#### §23.6.4 Failure branch

A green, value-defined declared outcome may carry a status-bearing value (e.g. `{status: 'failed'}`), but it is mirrored only after `workflowDone(childDef, childArts)` says the whole child is done. At that point the value propagates unchanged: the parent's calls: artifact greens with the failure status, and teardown (or other consumers) receives it through the normal green gate. A green failure outcome with outstanding child cleanup or a manual input keeps the parent mirror `owed`. Once the child is done, teardown runs on success AND failure — there is no special consume mode for failure.

#### §23.6.5 Gate fingerprint and re-arm

The machine-green fingerprint covers `gateStems` (the parent artifacts wired into the child via `callsInputs`) plus, as of F4 (§23.6.8), a reserved `__child_outcome_version__` key pinning the child outcome's version. The cross-instance join/re-green trigger is handled by `maintainCalls`'s outcome-value, `workflowDone`, and version-pin checks, not by green-fingerprint invalidation in the pure forward cascade: a calls: step still declares `consumes: []`, and its live green fingerprint must not be compared with the gate-only dependency set. Dead-end lint nevertheless treats every parent-artifact value in `callsInputs` as consumed; these values are lint consumers, not ordinary `consumes:` edges. Maintenance does use `callsInputs` separately to find dead gate branches, structurally skip the calls output and its downstream branch, and revive an engine-authored cascade skip after the gate returns green with moved versions. Human or producer skips retain the child-outcome pin semantics in §23.6.8 and are not revived merely because the gate is green.
...
Each mutating action does its **read-verify-write in ONE `BEGIN IMMEDIATE`**, not just its write. `maintainCalls`'s pre-reads (the outcome/done/gate/pin reads before STEP 6, the reverse-lookup before spawn) are an OPTIMISTIC gate that decides only *whether* to attempt the action — never what to commit; every value the write depends on is re-read and re-verified inside the same transaction. Under WAL this closes the cross-connection interleave where a second connection advances the parent gate (to a newer version), re-arms the child, or changes its completion state between the stale pre-read and the write: without it, a stale child value could be published under a fresh gate fingerprint, and the same tick would issue downstream work off that stale value. `publishCallsGreen` re-resolves the pinned child definition, verifies its expected output stem, and re-reads the parent artifact, all child artifacts, declared outcome, and gate in-tx; it requires the outcome still green/value-defined, the whole child still done, the gate still ready, the F4 pin passed, and child inputs consistent with the gate before computing the fingerprint and writing from that one snapshot. Any mismatch aborts (returns `false`, nothing written — no debt, no reasons, no events) and defers to the next tick. `rearmCallsGreen` performs the mirror in-tx recheck: it re-resolves the pinned child definition, verifies the expected output stem, reads all child artifacts, and re-arms only if the parent is still green and the outcome is non-green or the child is unfinished. The `commit`/`settled` events fire only when the helper actually wrote, so an aborted pass is silent. Child provisioning follows the same single-`BEGIN IMMEDIATE` shape (§23.6.9): `provisionCallsChild` re-reads the parent snapshot in-tx, then finds-or-creates the child and syncs its inputs from that snapshot — the reverse-lookup read, the insert, and every input write are one atomic step, so a child is never created (spawn) or re-written (re-provide) from a snapshot older than its committing tx. This is the creation-side twin of the machine-green fix above.
...
- The green-outcome/unfinished-child structural re-arm deletes this reserved pin only. The outcome version may remain unchanged while cleanup reruns, so retaining the pin would leave the mirror permanently owed after the child becomes done again. Gate-version entries remain intact, and non-green outcome, reject, and skip paths keep their existing pins.
- Publication stamps the outcome version from its authoritative in-transaction child snapshot; reject-propagation and skip use `Engine.childOutcomePin(parentWf, callsPath)` at verdict time. The structural re-arm operates on the already-stamped parent fingerprint and removes only that reserved key.
...
The `M2B-REARM` branch (§23.6.1 step 7) never lets the pin block reopening. A non-green child outcome retains it; a green outcome on an unfinished child clears it so the same output version may republish once the child finishes. In both cases, the parent returns to the live `owed` debt that deep descent follows.
...
**Tested depth and hard bound.** `calls: deep tick (2) three-level nesting — tick(root) drives the tree to done` covers three workflow instances driven to done from root ticks: the root at calls depth 0, a child at depth 1, and a grandchild at depth 2. `DEFAULT_MAX_CALL_DEPTH` is 64, and hosts may configure that bound with the `Engine` `maxCallDepth` option. The bound is defense in depth against a cycle that bypassed construction-time validation, not a normal tuning knob; there is no unlimited or arbitrary-depth guarantee. See also `calls: (REL-4) maxCallDepth stops a self-calling def cleanly — no overflow, bounded rows`.
