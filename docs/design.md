...
3. **Spawn**: if gate is ready and no child, `spawnChildIfAbsent(step.calls, parentWf, callsPath, gateValues)` — an atomic insert-or-read (§23.6.9) that either creates the child or re-attaches to one a concurrent tick already committed. Instance/`settled` events fire only when this call actually created the child. The parent calls: artifact stays `owed`. §23.6.7 (F2): a child input-schema refusal here is caught narrowly (`SchemaRefusalError` only — a genuine bug still throws) and recorded as a debt on the parent calls artifact instead of crashing the tick.
...
5. **Re-provide**: for each `callsInputs` mapping, if the parent's value differs (deep-equal) from what the child holds, `provideInput(child, inputName, newValue)`. The child re-runs internally. §23.6.7 (F2): same narrow schema-refusal handling as spawn — a child-illegal re-provided value becomes a debt on the parent calls artifact, not a thrown cascade.
...
The fix: `createInstance`/`provideInput` now throw a dedicated `SchemaRefusalError` (carrying the offending input name and schema issues) instead of a bare `Error` on a schema mismatch. `maintainCalls`'s STEP 3 (spawn) and STEP 5 (re-provide) each catch `SchemaRefusalError` narrowly — any other error still propagates, since that's a genuine bug, not an expected refusal. On catch, the parent calls artifact is recorded as a debt with the same shape `green()` uses for its own schema-reject branch: `acceptance: 'rejected'`, `schemaRejects + 1`, a `validation`-kind reasons entry naming the child input and the schema issues. The tick then proceeds instead of throwing.
...
- **Descent condition.** The parent descends into a child step only when all three hold: (1) the step's **gate is green** (same readiness `maintainCalls` uses to spawn/mirror — see `callsGateReady`), (2) a **child instance exists** (`findChildByParent`), and (3) the **debt is unpaid** (the parent's `calls:` artifact for that step is not green). A re-armed gate with a still-green mirror, or a step whose child was never spawned, is skipped — descent chases only real, outstanding child work. `callsDescendTargets(parentWf, def)` computes this set.
- **Own-tx-per-frame.** Descent runs *outside* the parent's transaction (`store.tx` is non-re-entrant — `BEGIN IMMEDIATE` would deadlock on itself). Each child frame opens its own tx via its own `tickInternal` call. The recursion carries a `visited` set so a diamond (two parents reaching the same child) or an accidental cycle ticks each instance at most once.
...
#### §23.6.9 REL-5 — atomic child creation (never two children per `calls:` step)

Before this fix, spawn read `findChildByParent` outside any transaction and then called `createInstance`, which opened its own transaction — a check-then-insert split across two tx boundaries and backed only by a *non-unique* reverse index. Two driver ticks (even in separate processes) could each observe "no child" and each insert one, leaving duplicate children for the same parent coordinate.

The fix makes the check and the insert one atomic step:

- **`spawnChildIfAbsent(defName, parentWf, callsPath, seedProvide)`** wraps `findChildByParent` + the insert in a *single* `BEGIN IMMEDIATE`. The first tick to acquire the write lock inserts the child; any other serializes behind it, re-reads the committed row, and re-attaches. It returns `{ id, created }`; the caller fires the `instance`/`settled` events only when `created` is true, so a re-attach (or a lost race) stays silent — matching the pre-REL-5 re-attach behavior. A `SchemaRefusalError` (a parent value illegal per the child's input schema) rolls the tx back with no orphan row, preserving the §23.6.7 F2 contract.
- **Partial UNIQUE index `workflow_produced_by_unique`** (schema version 8, on `(produced_by_wf, produced_by_path)` where both are `NOT NULL`) is the physical backstop that makes a duplicate impossible for any future writer path; `spawnChildIfAbsent` also catches the unique-constraint error and re-reads the winner as a belt-and-suspenders defense (unreachable under node:sqlite's write-lock-serialized connections).
