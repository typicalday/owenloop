...
Every command prints JSON on stdout; human-readable errors go to stderr.
...
| `lint` | validate every def in the defs dir; report errors and dead-end warnings |
| `check <def> [--assume-provided] [--format …] [--max-…]` | model-check a def's reachable state space |
...
`emit` / `seal`). `reject` can be born-rejected too — a
[judge's](authoring.md#judges---quality-gates-before-green)
